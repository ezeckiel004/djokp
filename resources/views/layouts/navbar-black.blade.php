...
<nav class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-transparent"
    ...
    :class="scrolled ? 'bg-black shadow-lg py-4' : 'bg-transparent py-6'">
    ...
    <div class="flex items-center justify-between px-4 lg:px-6">
        ...
        <div class="items-center hidden space-x-6 lg:flex">
            <!-- Accueil -->
            <a href="{{ url('/') }}" class="text-white transition duration-300 hover:text-yellow-400">
                Accueil
            </a>
            ...
            <a href="{{ route('formation.international') }}"
                class="text-white transition duration-300 hover:text-yellow-400">
                Formation à l'international
            </a>

            <!-- VTC & Transport -->
            <a href="{{ route('reservation') }}" class="text-white transition duration-300 hover:text-yellow-400">
                VTC & Transport
            </a>

            <!-- Location de voitures -->
            <a href="{{ route('location') }}" class="text-white transition duration-300 hover:text-yellow-400">
                Location de voitures
            </a>

            <!-- Conciergerie -->
            <a href="{{ route('conciergerie') }}" class="text-white transition duration-300 hover:text-yellow-400">
                Conciergerie
            </a>

            <!-- Blog -->
            <a href="{{ route('blog') }}" class="text-white transition duration-300 hover:text-yellow-400">
                Blog
            </a>
        </div>
        ...
        <div class="items-center hidden space-x-4 md:flex lg:hidden">
            ...
            <div class="relative" x-data="{ moreOpen: false }">
                <button @click="moreOpen = !moreOpen" @click.outside="moreOpen = false"
                    class="flex items-center space-x-1 text-white transition duration-300 hover:text-yellow-400 focus:outline-none">
                    <span>Plus</span>
        ...
        <!-- Desktop Auth Links (visible sur md et plus) -->
        <div class="items-center hidden space-x-4 md:flex">
            @if (Route::has('login'))
            <div class="text-right">
                @auth
                <a href="{{ url('/user/dashboard') }}" class="text-white transition duration-300 hover:text-yellow-400">
        ...
        <!-- Mobile Menu Button (visible sur petit écran) -->
        <div class="flex items-center md:hidden">
            <button @click="mobileMenuOpen = !mobileMenuOpen"
                class="p-2 text-xl text-white transition duration-300 focus:outline-none">

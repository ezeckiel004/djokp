...
<body class="h-screen flex">
    ...
    <div class="w-full lg:w-1/2 h-full flex justify-center bg-gray-50 animate-fade-in-left overflow-y-auto p-6 lg:p-8">
        <!-- CHANGEMENT CLÉ : items-center → justify-center + pt-32 pb-16 -->
        <div class="w-full max-w-md pt-2 pb-2" x-data="{ activeTab: 'login' }">
            ...
            <div x-show="activeTab === 'register'" class="tab-transition">
                <form method="POST" action="{{ route('register') }}" class="space-y-6">
                    ...
                    <!-- Détermination automatique du rôle_id pour client -->
                    <input type="hidden" name="role_id"
                        value="{{ App\Models\Role::where('slug', 'client')->first()->id ?? 3 }}">
                    ...
                    <label class="flex items-start text-sm text-gray-600">
                        <input type="checkbox" name="newsletter" value="1" {{ old('newsletter') ? 'checked' : '' }}
                            class="rounded text-green-500 focus:ring-green-500 w-5 h-5 mt-0.5">
                        <span class="ms-3">

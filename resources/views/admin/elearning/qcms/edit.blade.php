...
@section('title', 'Modifier le QCM | Admin DJOK PRESTIGE')

@section('content')
<div class="container max-w-6xl px-4 py-8 mx-auto">
    <div class="mb-6">
        <a href="{{ route('admin.elearning.qcms') }}" class="text-blue-600 hover:text-blue-800">
            <i class="mr-2 fas fa-arrow-left"></i> Retour aux QCM
        </a>
    </div>

    <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Modifier le QCM</h2>
        </div>

        <form action="{{ route('admin.elearning.qcms.update', $qcm->id) }}" method="POST" class="p-6" id="qcmForm">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-700">Titre du QCM *</label>
                    <input type="text" name="title" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        value="{{ old('title', $qcm->title) }}">
                    @error('title')
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-700">Cours associé</label>
                    <select name="cours_id"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="">Aucun (QCM indépendant)</option>
                        @foreach($cours as $coursItem)
                        <option value="{{ $coursItem->id }}" {{ old('cours_id', $qcm->cours_id) == $coursItem->id ?
                            'selected' : '' }}>
                            {{ $coursItem->title }}
                        </option>
                        @endforeach
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Description du QCM...">{{ old('description', $qcm->description) }}</textarea>
                @error('description')
                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-4">
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-700">Nombre de questions *</label>
                    <input type="number" name="questions_count" min="1" max="100" required readonly
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50"
                        value="{{ old('questions_count', $qcm->questions_count) }}" id="questionsCount">
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-700">Note minimale (%) *</label>
                    <input type="number" name="passing_score" min="0" max="100" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        value="{{ old('passing_score', $qcm->passing_score) }}">
                    @error('passing_score')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-700">Limite de temps (minutes)</label>
                    <input type="number" name="time_limit_minutes" min="0"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="Illimité" value="{{ old('time_limit_minutes', $qcm->time_limit_minutes) }}">
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-700">Tentatives autorisées</label>
                    <input type="number" name="attempts_allowed" min="1" max="10"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        value="{{ old('attempts_allowed', $qcm->attempts_allowed) }}">
            ...
            <div class="mb-6">
                <label class="flex items-center mb-4">
                    <input type="checkbox" name="is_examen_blanc" value="1"
                        class="text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ old('is_examen_blanc',
                        $qcm->is_examen_blanc) ? 'checked' : '' }}>
                ...
                <label class="flex items-center mb-4">
                    <input type="checkbox" name="allow_multiple_correct" value="1"
                        ...
                        old('allow_multiple_correct', $qcm->allow_multiple_correct) ? 'checked' : '' }}
                    id="allowMultipleCorrect">
                ...
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1"
                        class="text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ old('is_active',
                        $qcm->is_active) ? 'checked' : '' }}>
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <div class="text-sm text-gray-500">
                            <span id="questionCounter">{{ count($qcm->questions_data['questions'] ?? []) }}</span>
                            question(s)
                        </div>
                ...
                <div id="questionsContainer" class="space-y-6">
                    <!-- Les questions seront chargées depuis les données existantes -->
                </div>

                <!-- Champ caché pour les données JSON des questions -->
                <input type="hidden" name="questions_data" id="questionsData"
                    value="{{ old('questions_data', json_encode($qcm->questions_data ?? ['questions' => []])) }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const existingQuestions = @json($qcm->questions_data['questions'] ?? []);
        const allowMultiple = @json($qcm->allow_multiple_correct ?? false);

        // Initialiser la checkbox
        if (allowMultipleCorrectCheckbox) {
            allowMultipleCorrectCheckbox.checked = allowMultiple;
        }
        ...
        function createAnswerHTML(questionId, letter, text = '', isCorrect = false) {
            ...
            return `
                <div class="flex items-center p-3 space-x-3 border border-gray-200 rounded-lg answer-item bg-gray-50" data-letter="${letter}">
                    <div class="flex items-center space-x-2">
                        <input type="${inputType}" name="${inputName}"
                               class="w-4 h-4 text-blue-600 border-gray-300 correct-answer-input focus:ring-blue-500"
                               value="${letter}" ${isCorrect ? 'checked' : ''}>
                        <span class="w-4 text-sm font-medium text-gray-700">${letter}.</span>
                    </div>
                    <input type="text" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg answer-text focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                           placeholder="Texte de la réponse ${letter}" value="${text}">
                    <button type="button" class="text-red-600 transition-colors delete-answer hover:text-red-800" data-question-id="${questionId}" data-letter="${letter}">
        ...
        function addQuestion(questionData = null) {
            ...
            const currentAllowMultiple = allowMultipleCorrectCheckbox?.checked || false;

            const questionHTML = `
                <div class="p-6 bg-white border border-gray-300 rounded-lg question-card" data-question-id="${questionId}">
                    <div class="flex items-center justify-between pb-4 mb-6 border-b border-gray-200">
                        <div class="flex items-center space-x-3">
                            <div class="flex items-center justify-center w-8 h-8 font-semibold text-blue-800 bg-blue-100 rounded-full">
                                ${questionId}
                            </div>
                            <h4 class="font-medium text-gray-900 text-md">Question ${questionId}</h4>
                        </div>
                        <button type="button" class="text-red-600 transition-colors delete-question hover:text-red-800" data-question-id="${questionId}">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>

                    <div class="mb-6">
                        <label class="block mb-2 text-sm font-medium text-gray-700">Question *</label>
                        <input type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg question-text focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               placeholder="Entrez le texte de la question"
                               value="${questionData?.text || ''}">
                        <p class="mt-1 text-xs text-gray-500">Formulez clairement votre question</p>
                    </div>
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="flex items-center space-x-2">
                                <span class="text-xs text-gray-500" id="answer-type-hint-${questionId}">
                                    ${currentAllowMultiple ? 'Sélection multiple autorisée' : 'Une seule réponse correcte'}
                                </span>
                                <button type="button" class="text-sm text-blue-600 transition-colors add-answer-btn hover:text-blue-800" data-question-id="${questionId}">
                                    <i class="mr-1 fas fa-plus"></i> Ajouter une réponse
                                </button>
                            </div>
                        </div>

                        <div class="space-y-3 answers-container" id="answers-${questionId}">
                            <!-- Les réponses seront ajoutées ici -->
                        </div>
                    </div>

                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">Explication (optionnelle)</label>
                        <textarea class="w-full px-4 py-3 border border-gray-300 rounded-lg explanation focus:ring-2 focus:ring-blue-500 focus:border-transparent"
